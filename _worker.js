...
export default {
	async fetch(request, env, ctx) {
		...
		if (env.PROXYIP) {
			...
			启用反代兜底 = false;
		} else 反代IP = 'speed.cloudflare.com';
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (uuidRegex.test(请求UUID)) {
				...
				if (请求UUID === 目标UUID) return new Response(JSON.stringify({ Version: Number(String(Version).replace(/\D+/g, '')) }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
			}
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							if (!config_JSON.订阅转换配置.SUBAPI || config_JSON.订阅转换配置.SUBAPI.trim() === '') {
								return new Response('错误：默认订阅转换服务由于隐私安全原因已被禁用。若要使用 Clash/Sing-box 等格式的订阅，请在 Cloudflare Workers/Pages 控制台中添加环境变量 SUBAPI 并指定您信任的订阅转换后端（例如：https://sub.xeton.dev 或您自行搭建的后端）。', { status: 400, headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
							}
	...
	async function connecttoPry(允许发送首包 = true) {
		...
		const 当前连接任务 = (async () => {
			...
			if (启用SOCKS5反代 === 'socks5') {
				...
				newSocket = await connectProxyIP(host, portNum, 本次首包数据, 所有反代数组, 启用反代兜底);
			}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	try {
		...
		const defaultCopy = JSON.parse(JSON.stringify(默认配置JSON));
		if (!configJSON || 重置配置 == true) {
			await env.KV.put('config.json', JSON.stringify(defaultCopy, null, 2));
			config_JSON = defaultCopy;
		} else {
			config_JSON = 深度合并(JSON.parse(configJSON), defaultCopy);
		}
	} catch (error) {
		console.error(`读取config_JSON出错: ${error.message}`);
		config_JSON = JSON.parse(JSON.stringify(默认配置JSON));
	}
